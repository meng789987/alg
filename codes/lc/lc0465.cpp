...
public:
	...
	bool hasSubset(vector<int>& debts, size_t start, int setcnt, int selcnt, int selsum, vector<bool>& sel) {
		if (selcnt == setcnt) return selsum == 0;
		for (size_t i = start; i < debts.size(); i++) {
	...
	int dfs(vector<int>& debts, size_t start) {
		while (start < debts.size() && !debts[start]) ++start; // get next non-zero debt
		int res = INT_MAX;
		for (size_t i = start + 1, prev = 0; i < debts.size(); ++i) {
	...
	void test()
	{
		...
		cout << (11 == minTransfers(trans)) << endl;
		...
		cout << (16 == minTransfers(trans)) << endl;
	}
};
